...
m <- matrix(0, nrow = repeats, ncol = 3)

FHdied <- function(newtarget){
  m[trial, 2] <- m[trial, 2] - 1
  newtarget <- sample(c(1,3), 1)
  m[trial, newtarget] <- m[trial, newtarget] + 1
  }
...
#trying to make an ifelse to reroll the felhound death situation
```{r}
if_else(m[131, 2] > 6,
       function(x){
         m[131, 2] <- m[131, 2] - 1
         m[131, sample(c(1,3), 1)] <- m[131, sample(c(1,3), 1)] + 1},
       m[131, 2])
```

#step by step ifelse area T/F
```{r}
m[131, 2] > 6
```
